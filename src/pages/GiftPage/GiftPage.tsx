  ...
  const onCallback = (segment: SegmentType) => {
    if (segment.type === 'gift') {
      ...
      if (gift?.type === 'app-theme') {
        const lockedTheme = giftService.getRandomLockedGift(SettingsConstants.themes, unlockedThemes);
        if (lockedTheme) {
      ...
      if (gift?.type === 'roulette-colors') {
        ...
        if (lockedRoulette) {
          ...
          openModal({
            ...
            callbackOnClose: () => setIsAvailableToSpin(false),
            node: (
              <SmallRoulette segments={generateSegmentsMock(lockedRoulette.colors)} options={OPTIONS_SMALL_ROULETTE} />
            ),
            buttonText: 'Получить',
          });
        }
