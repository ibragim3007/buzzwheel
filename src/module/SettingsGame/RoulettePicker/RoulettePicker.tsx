...
export const generateSegmentsMock = (colors: [string, string, string]): SegmentType[] => {
  return [
    {
      color: colors[0],
      label: '',
...
export default function RoulettePicker() {
  ...
  const onRouletteColorChange = (color: IAvailableColor, isAvailable: boolean) => {
    if (isAvailable) {
  ...
  const initialScrollIndex = SettingsConstants.availableColors.findIndex(item => item.id === rouletteColor?.id) || 0;
        ...
        return (
          <Pressable onPress={() => onRouletteColorChange(item, isAvailable)}>
            ...
            >
              ...
              <SmallRoulette segments={generateSegmentsMock(item.colors)} options={OPTIONS_SMALL_ROULETTE} />
            </Grid>
          </Pressable>
        );
      }}
